...
namespace DSA.Assignment_12
{
    ...
    /// 
    /// </summary>
    public class HashTableQuadraticProbingCollision
    {
        static int size = 10;
        ...
        public void Run()
        {
            Console.WriteLine("Quadratic Probing Hashtable");
            ...
            Console.WriteLine();
            Console.WriteLine($"Get Value for Aug: {GetItem("Aug")}");
        }
        ...
        {
            var hashKey = QuadraticProbingHashFunction(keyValue.Key);
        ...
        public int QuadraticProbingHashFunction(string key)
        {
            for (int i = 0; i < size; i++)
            {
                int index = HashFunction(key);

                index = (index + (C * i) + (C * (i * i))) % size;
